...
public class UnsortedABPriQ<T> implements PriQueueInterface<T>  
                             
{
  ...
  public UnsortedABPriQ(Comparator<T> comp) 
  {
    elements = (T[]) new Object[DEFCAP];
    this.comp = comp;
  }
...
//      }
  ...
  {
    if (!isEmpty())
    { // Loop through array and find highest priority element
      // then remove that element
      
      int index = 0, highPriority = 0;
      for (int i = index; i < numElements - 1; i++)
         {  
            if (comp.compare(elements[i], elements[highPriority]) > 0)
               highPriority = i;
         }
         
      T temp = elements[highPriority];
      elements[highPriority] = null; // do we need to shift everything in queue to left?
      
      for (int i = highPriority; i < numElements - 1; i++)
         {
